...
class OpenFoodFactsService {
  constructor(country = "world") {
    ...
    this.protocol = "https";
    this.version = "v0";
    this.agent = `octacmsibmo - PWA - Version 0 - https://octacmsibmo.com - ${navigator.userAgent}`;
    this.foodInformationApi = `${this.protocol}://${this.country}.openfoodfacts.org/api/${this.version}`;
  }
  async get(ean) {
    try {
      const response = await fetch(`${this.foodInformationApi}/product/${ean}.json`, {
        headers: {
      ...
      const offResponse = await response.json();
...
class FoodService {
  constructor(StorageCreator) {
    this.storageService = new StorageCreator();
  }
  async getAll() {
    try {
  ...
  async add(item) {
    ...
    if (foods) {
      const newFood = Object.assign(Object.assign({}, item), { id });
      foods.push(newFood);
...
let AppRoot = class {
  ...
  render() {
    return (h("ion-app", null, h("ion-router", { useHash: false }, h("ion-route", { url: "/", component: "food-list", componentProps: { service: foodService } }), h("ion-route", { url: "/add", component: "scan-food", componentProps: { foodService, foodInformationService } }), h("ion-route", { url: "/profile/:name", component: "app-profile" })), h("ion-nav", null)));
  }
};
